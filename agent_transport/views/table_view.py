...
class AgentTransportTableView(TemplateView):
    ...
    def get_table(request):
        template_name = 'agent_transport/table1.html'
        ...
        if request.method == "GET":
            ...
            if not date:
                bookings = AgentTransport.objects.filter(Q(date__month=today.month) | (Q(return_tr='') & ~Q(cancel='1'))).order_by('date', 'work_id')
            else:
                bookings = AgentTransport.objects.filter(Q(date=date) | (Q(date__lte=today) & (Q(return_tr='') & ~Q(cancel='1')))).order_by('date', 'work_id')
        else:
            bookings = AgentTransport.objects.filter(date__month=today.month | (Q(return_tr='') & ~Q(cancel='1'))).order_by('date', 'work_id')
