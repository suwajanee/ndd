...
def api_get_agent_transports(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if date_filter == None:
                agent_transports = AgentTransport.objects.filter(Q(date=today) | Q(status__in=[1,3,4])).order_by('date', 'principal__name', 'shipper__name', 'work_type', \
                                    Case(
...
def api_filter_agent_transports(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            agent_transports = AgentTransport.objects.filter(**filter_dict).order_by('date', 'principal__name', 'shipper__name', 'work_type', \
                        Case(
                            When(work_type='ep', then='booking_no'),
                        ), 'work_id')
