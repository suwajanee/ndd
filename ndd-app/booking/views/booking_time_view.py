...
class BookingTimeView(TemplateView):
    ...
    def render_time_booking_page(request):
        ...
        if request.method == "POST":
            context['pk_list'] = request.POST.getlist("pk")
            context['date_filter'] = request.POST['date_filter']
            context['filter_by'] = request.POST['filter_by']

            context['bookings'] = Booking.objects.filter(pk__in = context['pk_list']).order_by('date', 'work_id')

            request.session['pk_list'] = context['pk_list']
            request.session['date_filter'] = context['date_filter']
            request.session['filter_by'] = context['filter_by']

            return render(request, template_name, context)
            # return render(request, template_name, {'bookings': bookings, 'filter_by': filter_by, 'date_filter':date_filter, 'today': today, 'context['tmr']': context['tmr'], 'nbar': 'booking-table'})
        else:
            if request.session['pk_list']:
                context['pk_list'] = request.session['pk_list']
                ...
                if not context['date_filter']:
                    ...
                    return redirect('booking-table' + '?filter_by=' + filter_by + '&date_filter=' + context['date_filter'])
